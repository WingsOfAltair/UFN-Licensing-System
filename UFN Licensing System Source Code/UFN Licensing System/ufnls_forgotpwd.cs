...
{
    public partial class ufnls_forgotpwd : Form
    {
        private string email, pwd, sq, sa, usq;

        Security Security = new Security();
        Connection Connection = new Connection();

        public ufnls_forgotpwd()
        {
            InitializeComponent();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            sa = textBox2.Text;
            pwd = Connection.RecoverPassword(email, sa, sq);
            if (pwd != "" && pwd != "Invalid answer.")
                MessageBox.Show("Your password is: " + pwd);
            else if(pwd == "Invalid answer.")
        ...
        private void ufnls_forgotpwd_Load(object sender, EventArgs e)
        {
            Security.CaptchaGraphic = Graphics.FromImage(Security.CaptchaImage);
